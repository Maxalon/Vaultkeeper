...
44. **`is:playtest` + default playtest hide, backed by `scryfall_cards.is_playtest`.** Scryfall marks playtest-subset cards with `promo_types: ['playtest']`. The Mystery Booster *Playtest* sets (cmb1, cmb2) carry it; so does the MB2 #501-#602 playtest subset which otherwise lives in a regular-looking `set_type='masters'` set. A per-card column populated at bulk sync time captures the signal — set_type/set_code heuristics can't (mb2 is masters, indistinguishable from mainline reprints).
    Soft filter hides any row with `is_playtest=true` by default. `is:playtest` (and `is:play-test`) filters to only those rows. The controller's hard set_type exclusion has an `OR is_playtest=1` escape hatch so CMB cards (which live in `set_type='funny'`, hard-excluded for Un-sets) can still be reached. Bang-exact match disables the hide.
    **Retry-on-empty**: if the first-pass query with default filters returns zero results AND defaults were actually applied, the controller silently re-runs the same query with `disable_defaults=true`. This surfaces hidden / playtest cards when the user's name search happens to match *only* those — a common case for cards that exist exclusively in eternal-style supplements or CMB playtest sets. A `warnings[]` entry (`"Normally-hidden cards shown because no other results matched."`) tells the user why their results include unusual cards.
    **Migration** (`2026_04_21_000003_add_is_playtest_to_scryfall_cards`) adds the column + seeds `is_playtest=true` for cmb1/cmb2 AND the MB2 #501-#602 range so the filter works immediately after deploy. Post-deploy, `scryfall:sync-bulk` replaces the seed values with the authoritative per-card `promo_types` signal — catching any future set's playtest subset automatically.
